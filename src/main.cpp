...
// Advertise as "Claude-XXXX" (last two BT MAC bytes) so multiple watches
...
bool    menuOpen    = false;
uint8_t menuSel     = 0;
uint8_t brightLevel = 4;           // 0..4 → halSetBrightness 等级
bool    btnALong    = false;

// 触摸手势状态
static int16_t touchStartX = -1, touchStartY = -1;
static uint32_t touchStartTime = 0;
static bool touching = false;
static bool touchConsumed = false;  // 滑动已消费，不再触发点击
// 物理按键状态（GPIO36）
...
static bool btnBPressed = false;     // 滑动触发
...
static void applyBrightness() { halSetBrightness(brightLevel); }
...
static void beep(uint16_t freq, uint16_t dur) {
  if (settings().sound) halBeep(freq, dur);
}
...
const uint8_t INFO_PG_CTFT_REDITS = 5;
...
void menuConfirm() {
  switch (menuSel) {
    ...
    case 1: halPowerOff(); break;
    case 2:
    case 3:
      menuOpen = false;
      displayMode = DISP_INFO;
      infoPage = (menuSel == 2) ? INFO_PG_BUTTONS : INFO_PG_CTFT_REDITS;
      applyDisplayMode();
      characterInvalidate();
      break;
...
// Clock orientation: gravity along the in-plane X axis means the watch is
...
//   1 = landscape, BtnA-side down (tft rotation 1)
//   3 = landscape, USB-side down (tft rotation 3)
...
static RTC_DateTime _clkTm;
uint32_t               _clkLastRead = 0;   // zeroed by data.h on time-sync
static bool            _onUsb       = false;
static void clockRefreshRtc() {
  if (millis() - _clkLastRead < 1000) return;
  _clkLastRead = millis();
  _onUsb = axp.getVbusVoltage() / 1000.0f > 4.0f;
  _clkTm = rtc.getDateTime();
}
...
static void drawClock() {
  ...
  // 秒数 1Hz 刷新；60fps 重绘 3 个字符串是 180 次 SPI 操作/秒，
  // 毫无意义。仅在秒数变化（或全屏重绘）时更新。
...
void drawInfo() {
  ...
  if (infoPage == 0) {
    ...
    spr.setTextColor(p.text, p.bg);    ln("swipe right");
    ...
    spr.setTextColor(p.text, p.bg);    ln("Power  side");
    ...
    int vBat_mV = (int)(axp.getBattVoltage());
    int iBat_mA = (int)axp.getBatteryChargeCurrent();
    int vBus_mV = (int)(axp.getVbusVoltage());
    ...
    spr.printf("%d%%", pct);
    spr.setTextSize(1);
    spr.setTextColor(full ? TFT_GREEN : (charging ? HOT : p.textDim), p.bg);
...
static void drawApproval() {
...
#else
  // Size 2 only if it fits one line (~20 chars at 12px on 240px screen)
  int toolLen = strlen(tama.promptTool);
  spr.setTextColor(p.text, p.bg);
  spr.setTextSize(toolLen <= 20 ? 2 : 1);
  spr.setCursor(4, H - AREA + (toolLen <= 20 ? 14 : 18));
  spr.print(tama.promptTool);
  spr.setTextSize(1);

  // Hint wraps at ~36 chars to two lines under the tool name
  spr.setTextColor(p.textDim, p.bg);
  ...
  if (hlen > 36) {
    ...
    spr.printf("%.36s", tama.promptHint + 36);
  }
#endif
  ...
  if (responseSent) {
    ...
    spr.setCursor(W - 72, H - 12);
    spr.print("swipe: deny");
  }
}
...
static void drawPetHowTo(const Palette& p) {
  ...
  ln(p.textDim, "touch screen = wake"); gap();

  ln(p.textDim, "tap: screens  swipe: page");
...
#else
  const int SHOW = 3, LH = 8, WIDTH = 36;
#endif
...
void loop() {
  ...
  if (activeState == P_ATTENTION && settings().led) {
    // 用背光闪烁替代 LED 脉冲
    if ((now / 400) % 2) {
      halSetBrightness(min(brightLevel + 1, 4));
    } else {
      halSetBrightness(brightLevel);
    }
  }
  ...
  // 触摸轮询
  uint8_t nPts = ts.getTouchPoints().getPointCount();
  if (nPts > 0 && !touching) {
    auto p = ts.getTouchPoints().getPoint(0);
    touchStartX = p.x;
    touchStartY = p.y;
    ...
    auto p = ts.getTouchPoints().getPoint(0);
    int16_t dx = p.x - touchStartX;
    int16_t dy = p.y - touchStartY;
    // 水平滑动检测（右滑 = BtnB）
    if (abs(dx) > 50 && abs(dx) > abs(dy) * 2) {
      touchConsumed = true;
      btnBPressed = true;
    }
  } else if (nPts == 0 && touching) {
    touching = false;
  ...
  if (btnALongPressed && !btnALong && !swallowBtnA) {
    btnALong = true;
    btnALongPressed = false;
    beep(800, 60);
